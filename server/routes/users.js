...
router.delete('/:email', async (req, res) => {
  try {
    console.log(' Delete account request for:', req.params.email);

    // Require API key
    const apiKey = req.get('X-API-Key') || req.get('x-api-key');
    const ADMIN_API_KEY = process.env.ADMIN_API_KEY || process.env.API_KEY || 'gita_admin_key_2024';
    if (!apiKey || apiKey !== ADMIN_API_KEY) {
      console.log(' Delete denied: invalid API key');
      return res.status(403).json({ success: false, error: 'Forbidden: invalid API key' });
    }

    // Require explicit confirmation (query ?force=1 or body { confirm: true })
    const confirm = (req.query.force === '1') || (req.query.confirm === 'true') || (req.body && req.body.confirm === true);
    if (!confirm) {
      return res.status(400).json({ success: false, error: 'Confirmation missing. Add ?force=1 or {confirm:true} to permanently delete.' });
    }
    ...
    // Soft-delete instead of removing from array
    users[userIndex].deleted = true;
    users[userIndex].deletedAt = new Date().toISOString();
    await db.users.save(users);

    // Update analytics: only count non-deleted users
    const analytics = await db.analytics.get();
    analytics.totalUsers = users.filter(u => !u.deleted).length;
    await db.analytics.save(analytics);

    console.log(' User soft-deleted:', req.params.email);

    res.json({ success: true, message: 'User soft-deleted. Use restore API to recover.' });
  } catch (error) {
...
// Restore endpoint
router.post('/:email/restore', async (req, res) => {
  try {
    const users = await db.users.getAll();
    const userIndex = users.findIndex(u => u.email === req.params.email);
    if (userIndex === -1) return res.status(404).json({ success: false, error: 'User not found' });

    if (!users[userIndex].deleted) return res.status(400).json({ success: false, error: 'User is not deleted' });

    users[userIndex].deleted = false;
    delete users[userIndex].deletedAt;
    users[userIndex].updatedAt = new Date().toISOString();
    await db.users.save(users);
