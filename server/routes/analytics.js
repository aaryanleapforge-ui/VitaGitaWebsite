...
router.get('/stats', async (req, res) => {
  try {
    ...
    const totalUsers = users.length;
        const shlokMap = new Map();
        // Use chapter + shlok as a unique key to deduplicate
        shloks.forEach(s => {
          const key = `${s.chapter}_${s.shlok}`;
          if (!shlokMap.has(key)) shlokMap.set(key, s);
        });
        const uniqueShloks = Array.from(shlokMap.values());
        // Force total shloks to 701 to match mobile app constant
        // (keeps admin dashboard consistent while we finalize shlok dataset)
        const totalShloks = 701; // uniqueShloks.filter(s => !(s.chapter == 0 || (s.chapterName && String(s.chapterName).trim() === '0.0'))).length;
